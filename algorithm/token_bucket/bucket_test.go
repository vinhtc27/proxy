...
func TestNewBucket(t *testing.T) {
	...
	if w, g := int64(0), b.Take(1); w != g {
		t.Fatal("Expected no filling when refillTime == -1")
	}
}
...
func TestBucket_Wait(t *testing.T) {
	...
	for b, wait := range cases {
		go func(b *bucket, wait time.Duration) {
			defer b.Close()
			...
			if int(wait.Seconds()) != int(got.Seconds()) {
				errors <- fmt.Errorf("bucket.Wait(2000) with cap=%d, refillTime=%s: Want: %s, Got %s",
					b.maxAmount, b.refillTime, wait, got)
			} else if took < wait-time.Second || took > wait+time.Second {
				// took is the actual time the b.Wait() took
				// wait is the time we expected it to take
				// if took is more than 1 second different from wait, then return an error
				errors <- fmt.Errorf("bucket.Wait(2000) with cap=%d, refillTime=%s: Waited for %v which isn't within 1 second of %v",
					b.maxAmount, b.refillTime, wait, took)
			} else {
				errors <- nil
			}
		}(b, wait)
